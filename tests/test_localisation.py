...
def test_workhorse_the_components_match_the_total_so_assets_is_odd():
    """filing R-1367: 93,840,769 against a total of 93,840,770."""
    item = _filing("93840769", "93840770", "93840770")
    assert feeder.localise(BOTH, item, ZERO) == "the assets figure is the odd one out"


def test_phi_group_the_components_match_assets_so_the_total_is_odd():
    """a filing of registrant R-4689 10-Q: components sum to its assets; the total is one below.

    The same defect class as Workhorse, pointing the other way. A single arm
...
def test_midnight_gaming_all_three_differ_so_nothing_is_claimed():
...
def test_enertopia_tagged_no_components_so_nothing_is_claimed():
    """a filing of registrant R-1046 10-Q: 500 out, and no second route exists for it."""
