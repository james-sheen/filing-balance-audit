...
So each in-range release goes into its own throwaway environment with this package
installed beside it, and the suite runs there. The highest release BELOW the floor goes
in too, because a floor nothing below it breaks is a floor nobody measured -- and the
floor in this package's `pyproject.toml` came from a design note, not a run. `--sweep`
keeps walking down until something fails, which is what turns the number into one.
...
Exit 0 every in-range release passed, 1 a declared claim is false, 2 the probe could not
run -- INCLUDING when a range holds no releases at all. *Every release passed* is true
of an empty set and means nothing, so an empty range is a failure to measure and not a
pass.

WHAT THIS DOES NOT DECIDE: whether a passing below-floor release means the floor should
drop. The pin is true either way. `floor-not-demonstrated` says only that this probe is
not where the number came from, and lowering it needs a reason a person supplies.
"""
...
def reap_abandoned(prefix: str = WORK_PREFIX,
    ...
    The `finally` in `probe` covers exceptions. It does not cover SIGKILL, a
...
#: `name>=lo,<hi`. Deliberately narrow: this package declares one pin and a
...
def exercise(dist: str, version: str, home: pathlib.Path) -> tuple[bool, str]:
    ...
    install = subprocess.run(
        [str(pip), "install", "-q", "--no-cache-dir", "pytest", f"{ROOT}[engine]"],
        capture_output=True, text=True)
    ...
    proof = subprocess.run(
        [str(python), "-c",
         f"import os, {module} as d, filing_balance_audit as p;"
